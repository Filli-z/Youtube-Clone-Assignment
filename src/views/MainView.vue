<template>
  <q-layout view="hHh lpR lFf">
    <q-header class="bg-white text-grey-8 q-py-md">
      <q-toolbar>
        ...
        <q-btn flat dense no-caps no-wrap>
          <svg
            ...
            viewBox="0 0 90 20"
            focusable="false"
            style="pointer-events: none; display: inherit; width: 100px; height: 50px"
          >
            <svg
        ...
        <q-space />
        <div class="q-pl-md YL__toolbar-input-container row no-wrap YL__search_field">
          <q-input borderless label="검색" class="col">
            <template v-slot:append>
              <q-icon v-if="$q.screen.gt.xs" class="q-pr-md" name="keyboard_alt" />
            </template>
          </q-input>
          <q-btn
            v-if="$q.screen.gt.xs"
        ...
        <q-btn text-color="dark" v-if="$q.screen.gt.xs" icon="mic" unelevated />
        <q-space />

        <div class="q-gutter-sm row items-center no-wrap">
          <q-btn dense flat icon="more_vert"> </q-btn>
          <q-btn rounded no-wrap padding="10px 10px" outline color="primary">
            <q-icon class="q-pr-md" name="account_circle" />
            <span v-if="$q.screen.gt.xs">로그인</span>
          </q-btn>
          <q-btn text-color="dark" round flat icon="invert_colors"> </q-btn>
        </div>
      </q-toolbar>
    </q-header>

    <q-drawer v-model="leftDrawerOpen" show-if-above :width="100">
      <div class="row">
        <q-btn
          class="col"
          v-if="!$q.screen.gt.xs"
        ...
        <q-btn class="col" v-if="!$q.screen.gt.xs" flat no-caps no-wrap>
          <svg
            xmlns="http://www.w3.org/2000/svg"
            id="yt-logo-updated-svg_yt3"
            class="external-icon"
      ...
      <q-item class="column" v-for="link in links1" :key="link.text" v-ripple clickable>
        <q-item-section class="col" avatar>
          <q-icon :name="link.icon" />
        </q-item-section>
        <p class="col">{{ link.text }}</p>
      </q-item>
    </q-drawer>
...
const links1 = ref([
  ...
  { icon: 'ondemand_video', text: '구독' },
  { icon: 'subscriptions', text: '시청기록' },
  { icon: 'restore', text: '시청기록' }
])
